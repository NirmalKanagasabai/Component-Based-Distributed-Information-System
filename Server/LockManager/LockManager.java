...
public class LockManager
{
                ...
                synchronized (this.lockTable) {
                    ...
                    if (!bConflict) {
                        ...
                        if (bConvert.get(0) == true) {
                            // lock conversion 
                            // *** ADD CODE HERE *** to carry out the lock conversion in the lccktable
    ...
    private void notifyWaitingThreads(DataObj dataObj) {

        // check if there are any waiting transactions.
        synchronized (this.waitTable) {
            // get all the transactions waiting on this dataObj
            Vector waitVector = this.waitTable.elements(dataObj);
            int waitSize = waitVector.size();
            for (int j = 0; j < waitSize; j++) {
                WaitObj waitObj = (WaitObj) waitVector.elementAt(j);
    ...
    public boolean LockConflict(DataObj dataObj, BitSet bitset) throws DeadlockException, RedundantLockRequestException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (dataObj.getXId() == dataObj2.getXId()) {
                ...
                if (dataObj.getLockType() == DataObj.READ) {    
                    ...
                    // if the transaction already has a read lock, set the bitset and just continue with execution.
                    // If no other transactions have a read lock, this will return false for no conflict detected
                    System.out.println("SEtting that bitset.");
                    bitset.set(0, true);

                    // if the transaction already has a write lock, it needs to throw a redundant lock request exception
